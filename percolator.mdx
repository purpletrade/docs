...
All four protocol repositories are open source. See the [Repositories](/repo-percolator-prog) section for detailed documentation on each component, or the [Programs & Deployments](/programs) page for on-chain addresses.

purple.trade's SDK has been verified byte-for-byte against the canonical source. See [Don't Trust, Verify](/verification) for the full audit.
