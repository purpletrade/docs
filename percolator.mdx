...
The Percolator engine is an open-source Solana program that implements perpetual futures entirely on-chain. Originally created by [Anatoly Yakovenko](https://github.com/aeyakovenko/percolator-prog), it stores complete market state in a single account called a **slab**.

Purple.trade deploys its own instance of the Percolator program and provides a full trading interface on top of it.
...
┌─────────────────────────────────────────┐
│ Header (72 bytes)                       │
│   magic, version, bump, admin           │
├─────────────────────────────────────────┤
│ MarketConfig (320 bytes)                │
│   collateral mint, oracle config,       │
│   margin thresholds, funding params     │
├─────────────────────────────────────────┤
│ Engine State (at offset 392)            │
│   open interest, funding accumulators,  │
│   crank state, risk parameters          │
├─────────────────────────────────────────┤
│ Account Slots (240 bytes each)          │
│   Up to 4096 user/LP accounts           │
│   Position data, collateral, PnL        │
└─────────────────────────────────────────┘
```
...
The canonical Percolator program source is open and available:

<Card title="percolator-prog" icon="github" href="https://github.com/aeyakovenko/percolator-prog">
  Single-file Rust program (~4,400 lines) implementing the complete perpetual futures engine.
</Card>

Purple.trade's SDK has been verified byte-for-byte against this source. See the [Source Verification](/verification) page for the full audit.
